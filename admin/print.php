...
<body>
    <div class="invoice-box">
        <?php
        $cid = intval($_GET['id']);
        $sql = mysqli_query($con, "select course.courseName as courname,course.courseCode as ccode,course.courseUnit as cunit,session.session as session,department.department as dept,level.level as level,courseenrolls.enrollDate as edate,session.semester as sem ,students.studentName as studentname,students.studentPhoto as photo,students.StudentRegno as sregno,students.creationdate as studentregdate from courseenrolls join course on course.id=courseenrolls.course join session on session.id=courseenrolls.session join department on department.id=courseenrolls.department join level on level.id=courseenrolls.level join students on students.StudentRegno=courseenrolls.StudentRegno join session on semester.id=courseenrolls.semester where courseenrolls.course='$cid'");
        $cnt = 1;
                ...
                <td colspan="2">
                    <table>
                        <tr>
                            <td class="title">
                                  <?php if ($row['photo'] == "") { ?>
                                    <img src="../student/studentphoto/nomage.png" width="200" height="200"><?php 
                                                                                                        } else { ?>
